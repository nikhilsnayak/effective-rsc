...
import { BrowserRenderer } from '../client/browser-renderer';
import { ClientRuntime } from '../client/client-runtime';
import { NavigationResources } from '../client/navigation-resource';
...
export const makeBrowserRefresh = Effect.all({
  ...
  Effect.map(({ browserNavigation, browserRenderer, navigationResources, refreshes, run }) => {
    ...
    const refreshInReactTransition = Effect.callback<void, BrowserRefreshError>(
      (resume, signal) => {
        startTransition(() =>
    ...
    return FiberMap.run(refreshes, CurrentRouteRefreshKey, refreshCurrentRoute).pipe(Effect.asVoid);
  }),
);
